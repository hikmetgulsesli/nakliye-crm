...
async function getClient(): Promise<OpenAI> {
  ...
  if (!apiKey) throw new Error('Groq API key yok — Sistem Ayarları > AI Sağlayıcılar ekranından ekleyin');
  if (!client || cachedKey !== apiKey) {
    client = new OpenAI({
      apiKey,
      baseURL: process.env.GROQ_BASE_URL || BASE_URL,
    });
    cachedKey = apiKey;
  }
  return client;
...
export const groqProvider: AIProvider = {
  name: 'groq',
  get defaultModel() {
    return defaultModel('groq');
  },

  async isConfigured() {
    const key = await getSecret('groq_api_key', 'GROQ_API_KEY');
    return Boolean(key);
  },
